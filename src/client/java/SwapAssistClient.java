...
public class SwapAssistClient implements ClientModInitializer {
    ...
    private static void renderSlotOverlays(GuiGraphicsExtractor graphics, Minecraft client) {
        ...
        // ATTRIBUTE SLOT
        if (swapMode.usesFixedAttributeSlot && fixedAttributeSlot != Inventory.NOT_FOUND_INDEX) {

            Identifier texture =
                    selectedSlot == fixedAttributeSlot
                            ? HOTBAR_SELECTION_ATTRIBUTE
                            : ATTRIBUTE_SELECTION;

            graphics.blitSprite(
                    RenderPipelines.GUI_TEXTURED,
                    texture,
                    x + fixedAttributeSlot * 20 - 1,
                    y - 1,
                    24,
                    24
            );
